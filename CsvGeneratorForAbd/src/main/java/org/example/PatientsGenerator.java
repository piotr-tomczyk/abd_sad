...
public class PatientsGenerator {
    ...
    public PatientsGenerator(String fileName) throws IOException {
        this.writer = new WriterToCsvFile(fileName);
    }

    public void generatePatients(int howMany) {
        List<Object> columns = List.of("pesel", "name", "surname", "email", "phone_num", "born", "address", "disability", "medical_offer");
        ...
        List<String> medOff = List.of("STANDARD", "SENIOR");


        for (int i = 0; i < howMany; i++) {
            List<Object> patient = new ArrayList<>();
            patient.add(peselsList.get(i));
            patient.add(faker.name().firstName());
            patient.add(faker.name().lastName());
            patient.add(emailsList.get(i));
            patient.add(phoneNumsList.get(i));
            patient.add(getFormattedBorn(mock, start, stop, dateFormatter));
            patient.add(faker.address().streetAddress());
            patient.add((int) Math.round(Math.random()));
            patient.add("None");

            writer.writeLineToFile(patient);
        }
